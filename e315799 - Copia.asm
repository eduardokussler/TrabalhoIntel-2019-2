...
leLinha:
    ;le as linhas do arquivo e desenha os ladrilhos adequadamente
    mov al, alturaTemp
...
loopLimpatec:
        ...
        mov [bx],  ah;zera o buffer na posicao atual
...
achouBarraZero1: ;quando achar o '\0', adiciona 
                ;a extensao .rel ao nome do arquivo
                mov al , '.'
                mov BYTE PTR[bx], al
                inc bx
                mov al, 'r'
...
trocaExtensao proc near
        ...
        mov ah, tamanhoNomeArquivo
testaTamanho:
        cmp ah, 4 ;se ax for 4, significa que resta os caracteres par'\0' no array
        je colocaRel
        inc bx
        dec ah
        jmp testaTamanho
...
ajeitaPosicaoQuad proc near
    push bx
    mov bx, 24
   ; mov bx, WORD PTR tamanhoQuad ;coloca o tamanho do quadrado a ser desenhado
    mov tamanhoLadoH, bx
    mov tamanhoLadoV, bx
    mov corTemp, al
...
;recebe a altura pela variavel altura
;recebe a largura pela variavel largura

calcTamanho proc near
    push ax
    mov ax, alturaDisp
    div BYTE PTR altura ;divide 360(altura max disponivel para desenho) pelo numero de linhas
    mov resTmp, al

    mov ax, larguraDisp
    div BYTE PTR largura ;divide 624(largura max disponivel pra desenho) pelo numero de colunas
    cmp al, resTmp
    jp moveLarg
    jmp moveAlt
fimCalcTamanho:
